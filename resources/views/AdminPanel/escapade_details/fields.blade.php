@isset($escapadedetail)
@method('PUT')
<input type="hidden" value="{{ $escapadedetail->id }}" name="id">
@endisset
@csrf
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-12 fv-row fv-plugins-icon-container">
                            <input type='text' name="{{$name}}[title]"
                                ...
                                value="{{isset($escapadedetail) && isset($escapadedetail->getTranslation($name)->title) ? $escapadedetail->getTranslation($name)->title : '' }}">
                            <div
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-12 fv-row fv-plugins-icon-container">
                            <textarea name="{{$name}}[description]" class="summernote"
                                ...
                                {{ isset($escapadedetail) ? $escapadedetail->value : '' }}
                                {{ isset($escapadedetail) ? $escapadedetail->getTranslation($name)->description : '' }}
                            </textarea>
                            <div
                                class="fv-plugins-message-container fv-plugins-message-container--enabled invalid-feedback">
                            </div>
...
<script>
    $('.delete').each(function(e){
        $(this).on('click',function(){
            let button = this;
            $.ajax({
            url :"{{ url(LaravelLocalization::getCurrentLocale().'/escapade-details/image/delete/') }}/" + this.getAttribute('data-image-id'),
            type: 'GET',
            headers: {
            '__token':'{{csrf_token()}}',
            },
