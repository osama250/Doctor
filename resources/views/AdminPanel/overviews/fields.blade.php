            ...
            <div class="row mb-3">
                ...
                <div class="col-lg-8">
                    ...
                    <div class="row">
                        ...
                        <textarea name="{{$name}}[title]" class="summernote"
                                class="form-control form-control-lg form-control-solid mb-3 mb-lg-0 "
                                placeholder="{{ __('lang.value') }}" >
                                {{ isset($overview) ? $overview->value : '' }}
                                {{ isset($overview) ? $overview->getTranslation($name)->title : '' }}
                        </textarea>
                        </div>
                        <!--end::Col-->
                    </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-lg-8">
                    ...
                    <div class="row">
                        <!--begin::Col-->
                        <div class="col-lg-12 fv-row fv-plugins-icon-container">
                            <input type='text' name="{{$name}}[subtitle]"
                                class="form-control form-control-lg form-control-solid mb-3 mb-lg-0 "
                                placeholder="{{ __('lang.sub_title') }}"
                                value="{{isset($overview) && isset($overview->getTranslation($name)->subtitle) ? $overview->getTranslation($name)->title : '' }}">
                            <div
  ...
  <div class="fv-row mb-8">
    ...
    <div class="dz-clickable dropzone" id="kt_modal_create_project_settings_logo">
        ...
        <div class="dz-message needsclick">
            ...
            @if( isset($page) && count($page->photos) > 0 )
                @foreach ($page->photos as  $photo)
                    <div class="dz-preview dz-processing dz-image-preview dz-success dz-complete">
                        <div class="dz-image"><img data-dz-thumbnail="" alt="{{$page->title}}"
                            {{-- src="/image/upload/pages/{{$photo->photo}}" > --}}
                            {{-- src="/image/upload/pages/' .$photo->photo" > --}}
                            src="{{ asset('upload/pages/'.$photo->photo ) }}" >
                        </div>
...
<script>
    ...
    var myDropzone = new Dropzone("#kt_modal_create_project_settings_logo", {
        url: '{{url(LaravelLocalization::getCurrentLocale()."/image/upload/overview")}}',
        paramName: "photo",
        //maxFilesize: 2,
...
<script>
    $('.delete').each(function(e){
        $(this).on('click',function(){
            let button = this;
            $.ajax({
            url :"{{ url(LaravelLocalization::getCurrentLocale().'/overview/image/delete/') }}/" + this.getAttribute('data-image-id'),
            type: 'GET',
            headers: {
            '__token':'{{csrf_token()}}',
            },
