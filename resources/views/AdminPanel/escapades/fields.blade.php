@isset($escapade)
@method('PUT')
<input type="hidden" value="{{ $escapade->id }}" name="id">
@endisset
@csrf
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-12 fv-row fv-plugins-icon-container">
                            <input type='text' name="{{$name}}[title]"
                                ...
                                value="{{isset($escapade) && isset($escapade->getTranslation($name)->title) ? $escapade->getTranslation($name)->title : '' }}">
                            <div
...
<script>
    ...
    var myDropzone = new Dropzone("#kt_modal_create_project_settings_logo", {
        url: '{{url(LaravelLocalization::getCurrentLocale()."/image/upload/")}}',
        paramName: "photo",
        //maxFilesize: 2,
...
<script>
    $('.delete').each(function(e){
        $(this).on('click',function(){
            let button = this;
            $.ajax({
            url :"{{ url(LaravelLocalization::getCurrentLocale().'/escapades/image/delete/') }}/" + this.getAttribute('data-image-id'),
            type: 'GET',
            headers: {
            '__token':'{{csrf_token()}}',
            },
