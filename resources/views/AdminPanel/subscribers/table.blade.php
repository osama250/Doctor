...
<table id="kt_datatable_dom_positioning" class="table table-striped table-row-bordered gy-5 gs-7 border rounded">
        ...
        <div class="col-lg-6">
            ...
            <div class="row">
                ...
                <div class="col-lg-12 fv-row fv-plugins-icon-container">
                    <select class="form-select form-select-solid filter ml-lg-5 ml-md-5 w-50"
                        id="kt_datatable_search_status" data-control="select2" data-hide-search="true"
                        ...
                        <option value="all" data-select2-id="select2-data-74-kng8" selected>{{__('lang.selectall')}}
                        </option>
                        <option value="{{__('lang.active')}}" data-select2-id="select2-data-74-kng8">
                            {{__('lang.active')}}</option>
                        <option value="{{__('lang.inactive')}}" data-select2-id="select2-data-131-9yfh">
    ...
    <tbody>
        ...
        <tr>
            <td>
                <span class="badge badge-light-success fs-7 fw-bold">{{ $subscriber->email }}</span>
            </td>

             <td class="no-export">
                {{-- @if(auth()->user()->can('Edit User'))
                <a href="{{ route('users.edit', $user->id) }}"
                    class="btn btn-sm btn-light me-2 {{ $user->status==1? 'bg-danger' :'bg-success' }}">
                    @if ($user->status == 1)
                ...
                <a class="btn btn-sm btn-light me-2 " data-bs-toggle="modal" data-bs-target="#emailmodal{{$subscriber->id}}">
         ...
         <!-- Modal -->
        <div class="modal fade" id="emailmodal{{$subscriber->id}}" tabindex="-1" role="dialog"
            aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">{{__('lang.sendemail').$subscriber->email}}</h5>
                    ...
                    <div class="modal-body">
                        <form id="kt_account_profile_details_form"
                            ...
                            enctype="multipart/form-data" action="{{ route('subscribers.store') }}" method="POST">
                            @csrf
                            <input type='hidden' value="{{$subscriber->email}}" name='email'>
                            <!--begin::Input group-->
                            <div class="row mb-6">
                                <!--begin::Label-->
